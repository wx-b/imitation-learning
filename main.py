...
def main(parameterisation=None):
  # Setup
  ...
  # Return mean and standard error of last evaluation
  return np.array(metrics['test_returns'][-1]).mean(), np.array(metrics['test_returns'][-1]).std() / sqrt(args.evaluation_episodes)
